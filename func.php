...
if(isset($_POST['patsub'])){
	...
	$query="select * from patient_tbl  where email='$email' and password='$password';";
...
if(isset($_POST['update_data']))
{
	...
	$status=$_POST['status'];
	$query="update appointment_tbl set payment='$status' where contact='$contact';";
	$result=mysqli_query($con,$query);
	if($result)
...
if(isset($_POST['doc_sub']))
{
  ...
  $docFees=$_POST['docFees'];
	$query="insert into midwife_tbl (username,password,email,docFees)values('$doctor','$dpassword','$demail','$docFees')";
	$result=mysqli_query($con,$query);
	if($result)
...
<?php 
	...
	// // $sql="select * from patient_tbl where DATE_FORMAT(child_birth, '%m-%d')='{$current_month_day}'";
	// $sql="select * from appointment_tbl where DATE_FORMAT(appdate, '%m-%d')='{$current_month_day}'";
	// $res=$con->query($sql);
	// if($res->num_rows>0){
	...
	// 		$notifications[]="<i class='fa fa-bell'></i> Wish <b>{$row["fname"]}</b> a Happy Birthday!<br> This is <b>{$row["fname"]}</b>'s ".number_suffix($age)." Birthday. date of birth is <b>".date("d-m-Y",strtotime($row["appdate"]))."</b>";
	// 	}
	// }
?>
