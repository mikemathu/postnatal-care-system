...
if(isset($_POST['docsub1'])){
	...
	$query="select * from midwife_tbl  where username='$dname' and password='$dpass';";
    ...
    echo("<script>alert('Invalid Username or Password. Try Again!');
          window.location.href = 'http://localhost/postnatal-care-system/index.php';</script>");
  }
}
...
function display_antigen()
{
 global $con;
 $query = "select * from immunization_tbl ";
 $result = mysqli_query($con,$query);
 while( $row = mysqli_fetch_array($result) )
 {
  $antigen = $row['antigen'];
  echo '<option value="' .$antigen. '">'.$antigen.'</option>';
 }
}
...
function display_patients()
{
 ...
 while( $row = mysqli_fetch_array($result) )
 {
  ...
  // $price = $row['docFees'];
  // $spec = $row['spec'];
  // echo '<option value="' .$fname. '" data-value="'.$price.'" data-spec="'.$spec.'">'.$fname.'</option>';
  echo '<option value="' .$fname.'"  data-pid="'.$pid.'" data-lname="'.$lname.'" data-gender="'.$gender.'" data-email="'.$email.'" data-contact="'.$contact.'">'.$fname.' </option>';
...
function display_specs() {
  global $con;
  $query="select distinct(spec) from midwife_tbl ";
  $result=mysqli_query($con,$query);
  while($row=mysqli_fetch_array($result))
  {
    $spec=$row['spec'];
...
function display_age()
{
 ...
 while( $row = mysqli_fetch_array($result) )
 {
  ...
  $child_age = $row['age'];
  $antigen = $row['antigen'];
  $pdisease = $row['disease_prevented'];
  // echo '<option value="' .$age. '">'.$age.'</option>';
  // echo '<option value="' .$age. '" data-antigen="'.$antigen.'" data-disease="'.$disease.'">'.$age.'</option>';


  echo '<option value="' .$child_age.'"  data-antigen="'.$antigen.'" data-pdisease="'.$pdisease.'">'.$child_age.' </option>';

  // echo '<option value="' .$child_age.'" data-pid="'.$ID.'"  data-lname="'.$pdisease.'"  data-email="'.$antigen.'">'.$child_age.' </option>';

 }
}

function display_disease_prevented()
{
 global $con;
 $query = "select * from immunization_tbl ";
 $result = mysqli_query($con,$query);
 while( $row = mysqli_fetch_array($result) )
 {
  $disease_prevented = $row['disease_prevented'];
  echo '<option value="' .$disease_prevented. '">'.$disease_prevented.'</option>';
 }
}
...
// {
// 	$name=$_POST['name'];
// 	$query="insert into midwife_tbl (name)values('$name')";
// 	$result=mysqli_query($con,$query);
// 	if($result)
