...
<html lang="en">
  ...
  <a class="navbar-brand" href="#"><i class="fa fa-user-plus" aria-hidden="true"></i> Postnal Care </a>
...
if(isset($_POST["send"])){
    ...
    $username = 'kishan';
    // $sql = "INSERT into messageInbox_tbl  (sender, receiver, msg) VALUES ('$username', '$msgTo', '$msgBody')";

    $query="insert into messageInbox_tbl (sender, receiver, msg)values('$username', '$msgTo', '$msgBody')";
    $result=mysqli_query($con,$query);
...
// $sql = "SELECT * from messageInbox_tbl  WHERE receiver='$username' ORDER BY timestamp DESC";
// $result = $conn->query($sql);

$query = "select * from messageInbox_tbl  where receiver=$username order by timestamp";
$result = mysqli_query($con,$query);
...
if(isset($_POST["s_inbox"])){
	$t=$_POST["s_inbox"];
	$sql = "SELECT * from messageInbox_tbl  WHERE receiver='$username' and sender='$t' ORDER BY timestamp DESC";
	$result = $conn->query($sql);
	$f=0;
}
...
if(isset($_POST["s_sm"])){
	$t=$_POST["s_sm"];
	$sql = "SELECT * from messageInbox_tbl  WHERE sender='$username' and receiver='$t' ORDER BY timestamp DESC";
	$result = $conn->query($sql);
	$f=1;
}
...
if(isset($_POST["sm"])){
	$sql = "SELECT * from messageInbox_tbl  WHERE sender='$username' ORDER BY timestamp DESC";
	$result = $conn->query($sql);
	$f=1;
}
...
<!--Client Profile Details-->	
		<div class="card" style="padding:20px 20px 5px 20px;margin-top:-50px">
        ...
        $conn=mysqli_connect("localhost","root","","pmsdb");
                    ...
                    $midwife_id = $row['midwife_id'];
                                ...
                                <input type="hidden" name="sr" value="rrrr">
                                    ...
                                    <a style="color: #fff; background-color: #342ac1; border-color: #342ac1; margin: 1px 0px padding:0px!important" href="reply2.php?id=<?php echo $midwife_id; ?>" type="submit" class="btn btn-primary">
                                    Reply
                                    </a>
                                   <?php
                                   '
